...
size_t build_sas_token(const char *key_b64, const char *resource_uri,
    ...
    uint8_t key_raw[128];
    size_t key_raw_len = azureiot_base64_decode(key_b64, strlen(key_b64), key_raw, sizeof(key_raw));
    if (key_raw_len == 0) return 0;

    char uri_enc[256];
    azureiot_url_encode(resource_uri, uri_enc, sizeof(uri_enc));
    ...
    azureiot_hmac_sha256(key_raw, key_raw_len, (const uint8_t *)to_sign, (size_t)n, mac);

    char sig_b64[64];
    size_t sig_b64_len = azureiot_base64_encode(mac, SHA256_BLOCK_SIZE, sig_b64);
    sig_b64[sig_b64_len] = '\0';

    char sig_enc[128];
    azureiot_url_encode(sig_b64, sig_enc, sizeof(sig_enc));
