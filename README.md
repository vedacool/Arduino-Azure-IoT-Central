...
- **A third real bug found during this full-codebase audit pass**: `ensureMqttConnected()`'s `username` buffer had only 4 bytes of real margin against the same worst-case field lengths as the `resourceUri` bug above -- not currently broken (the numbers work out today), but fragile against any future change to `s_iotHubHost`/`s_deviceId`'s sizes, and it was the one remaining `snprintf` in the file without a truncation check. Added one for consistency and future-proofing, even though no truncation was actually reproducible today.
- **The twin `$version` extraction was verified against real Azure twin JSON, not assumed**: checked 6+ authoritative examples (Microsoft Q&A, Azure IoT SDK for Java's own documentation, Azure Samples) to confirm `"$version"` (lowercase, exact) appears exactly once per section (`desired`/`reported`), always as a sibling of `$metadata`, never nested inside it -- `$metadata` uses the differently-named `$lastUpdatedVersion` for per-property tracking instead, so there's no risk of `extractJsonNumber()`'s first-occurrence search picking up the wrong version number from a nested per-property metadata block.
